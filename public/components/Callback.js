...
class Callback extends React.Component {
	...
	getPermissions = (token) => {
		console.log("Callback.js, getPermissions");
		this.setState({ loading: true, error: null });
		if (process.env.PERMISSIONS_DISABLED === "true") {
			this.checkSuccess();
			console.log("Permissions are disabled.");
		} else {
			getData(`${process.env.API_URL}/api/v1.0/oidc/permissions`, token)
			.then((data) => {
	...
	checkSuccess = () => {
		console.log("Callback.js, checkSuccess");
		if (
...
componentDidMount() {
	console.log("Callback.js, componentDidMount");
	...
	if (params.has("refresh_token")) {
		document.cookie = `REFRESH_TOKEN=${params.get(
			"refresh_token",
			)}; SameSite=None; Secure; HttpOnly; Path=/api/v1.0/oidc/refresh`;
	}
...
render() {
	console.log("Callback.js, render: yes");
	return (
